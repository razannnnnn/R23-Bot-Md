...
async function startRazan() {
  ...
  const conn = connConnect({
    ...
    browser: ["R23Bot Multi Device", "Safari", "1.0.0"],
  ...
  conn.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      require("./razan")(conn, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  conn.ev.on("group-participants.update", async (anu) => {
    ...
    try {
      ...
      for (let num of participants) {
        ...
        if (anu.action == "add") {
          let a = `*Hello @${num.split("@")[0]}, Welcome To ${
            metadata.subject
          ...
          let a = `Congratulations @${
            num.split("@")[0]
          }, on being promoted to admin of this group ${metadata.subject} 🎉`;
          conn.sendMessage(anu.id, {
            text: a,
            contextInfo: {
              externalAdReply: {
  ...
  conn.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        console.log(
          `[BOT] Bad Session File, Please Delete Session and Scan Again`
        );
        startRazan();
        ...
        conn.end(`[BOT] Unknown DisconnectReason: ${reason}|${connection}`);
    }
    if (
